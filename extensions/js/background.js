/*
* 用于处理来自 content-script 的消息，将消息转发给 http.js 处理，然后将处理结果返回给 content-script。
*/
...
const http = {
    request: function (options) {
        ...
        if (typeof _data == "string") {
            requestOptions.headers["Content-type"] = "text/plaincharset=utf-8"
            ...
            if (Object.keys(requestOptions.data).some(key => {
                ...
                requestOptions.headers["Content-type"] = "application/jsoncharset=utf-8"
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    new Promise(async (resolve, reject) => {
        ...
        switch (message.type) {
            ...
            case 'FetchPostForm': {
                http.postForm(message.data).then(response => {
                    resolve(parseHttpResponse(response))
                }).catch(error => {
                    reject(parseHttpResponse(error))
                })
